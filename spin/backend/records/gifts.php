  ...
  <thead>
    <tr class="table-danger">
      ...
      <td>Action</td>
            ...
            <?php $res=mysqli_query($connect,"SELECT *  FROM commodities_legal_week where region='$region'  ORDER BY id ASC") or die(mysqli_error($connect)); 
//WHERE region='$sent_loc'
            while($row=mysqli_fetch_array($res))
 {
    $name=$row['name'];
    $id=$row['id'];
    $amount_remaining=$row['remaining_items'];
    $pic=$row['name'];
    $region=$row['region'];
    ?>
<tr>
